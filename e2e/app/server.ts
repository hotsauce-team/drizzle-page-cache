// Minimal app for the e2e purge loop: node:sqlite + drizzle + this package.
...
const pageCache = createPageCache({
  ...
  settleMs: 10,
  purge: souinPurger(
    Deno.env.get("PURGE_URL") ?? "http://caddy/souin-api/souin",
  ),
});
...
Deno.serve({ port: 8000, hostname: "0.0.0.0" }, pageCache.middleware(app));
